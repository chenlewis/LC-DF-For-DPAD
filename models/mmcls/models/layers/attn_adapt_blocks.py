...
class CEABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
             drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_ratio_search=1.0,adapter_dim=16,scale_factor=8 ):
        super().__init__()
    ...
    def forward(self,x_otsu,z_otsu, x,z,x_origin,z_origin,adapter_up,i, global_index_template, global_index_templatei, global_index_search, global_index_searchi,
        ...
        z = z + self.drop_path(self.mlp(self.norm2(z)))+self.drop_path(self.adap2_t(self.norm2(xori))) + self.drop_path(
...
class CEABlock_EVP(nn.Module):
    ...
    def forward(self, x_otsu, z_otsu,x_FMAG, x, z, x_origin, z_origin, adapter_up,adapter_up2, i, global_index_template,
...
class CEABlock_FMAG(nn.Module):
    ...
    def forward(self, x_otsu, z_otsu,x_FMAG, x, z, x_origin, z_origin, adapter_up, mask_x=None,
                mask_z=None):
        ...
        if x_FMAG is not None:
            x_FMAG = adapter_up(self.adap_st(self.norm1(x_FMAG)))
            x = torch.cat((
                x[:, :1, :],
                x_FMAG + x[:, 1:, :],
            ), dim=1)
