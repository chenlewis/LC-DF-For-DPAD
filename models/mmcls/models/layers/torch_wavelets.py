...
class DWT_Function(torch.autograd.Function):
    ...
    def forward(ctx, x, w_ll, w_lh, w_hl, w_hh):
        ...
        # Match weights dtype with input x dtype
        ...
        # 拼接子带恢复原始分辨率
        top = torch.cat([x_ll, x_lh], dim=-1)  # 水平方向拼接
        bottom = torch.cat([x_hl, x_hh], dim=-1)  # 水平方向拼接
        x = torch.cat([top, bottom], dim=-2)  # 垂直方向拼接
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            # Match weights dtype with input dx dtype
            w_ll = w_ll.to(dx.dtype)
            w_lh = w_lh.to(dx.dtype)
            w_hl = w_hl.to(dx.dtype)
            w_hh = w_hh.to(dx.dtype)
...
class DWT_2D(nn.Module):
    ...
    def forward(self, x):
        # Apply DWT and return spatially concatenated result
        x = DWT_Function.apply(x, self.w_ll, self.w_lh, self.w_hl, self.w_hh)
        return x
...
class IDWT_Function(torch.autograd.Function):
    ...
    def forward(ctx, x, filters):
        ...
        # Match filters dtype with input x dtype
    ...
    def backward(ctx, dx):
        if ctx.needs_input_grad[0]:
            ...
            # Match filters dtype with input dx dtype
            filters = filters.to(dx.dtype)
...
class IDWT_2D(nn.Module):
    def __init__(self, wave):
        ...
        w = pywt.Wavelet(wave)
        rec_hi = torch.Tensor(w.rec_hi)  # High-pass reconstruction filter
        rec_lo = torch.Tensor(w.rec_lo)  # Low-pass reconstruction filter

        w_ll = rec_lo.unsqueeze(0) * rec_lo.unsqueeze(1)  # LL (Low-Low)
        w_lh = rec_lo.unsqueeze(0) * rec_hi.unsqueeze(1)  # LH (Low-High)
        w_hl = rec_hi.unsqueeze(0) * rec_lo.unsqueeze(1)  # HL (High-Low)
        w_hh = rec_hi.unsqueeze(0) * rec_hi.unsqueeze(1)  # HH (High-High)
