...
from ..layers.patch_embed import PatchEmbed,PatchEmbed_prompt,PatchEmbedWithFrequencyDropout,PatchEmbed_filter,FilterSelector
...
from ..layers.attn_adapt_blocks import CEABlock_FMAG as CEABlock  ##BAT
...
def load_dict_params(state_dict, model, matched_keys, weights):
    ...
    for k, v in state_dict.items():
        ...
        if isinstance(v, dict):
            ...
            # 如果v是tensor类型，继续正常处理
            close_matches = get_close_matches(k, [i for i in model.state_dict()], n=1, cutoff=0.4)
...
default_cfgs = {
    ...
    'vit_base_patch16_224': _cfg (url= None,  # 可以是URL，也可以是None,
    pretrained_finetune='/home/lyj/code_lpq/pretrain_finetune/vit_base_p16_224-4e355ebd.pth',),
    'vit_base_patch16_224_FMAG': _cfg(url=None,  # 可以是URL，也可以是None,
    pretrained_finetune='/home/data1/lyj/CMA/Output_RODHQ/FMAG/ViT/ViT-0.pth',),
    'vit_base_patch16_224_FMAG_DM': _cfg(url=None,  # 可以是URL，也可以是None,
    pretrained_finetune='/home/lyj/MFM/models/ViTB16_FAG.pth', ),
    'beit_base_patch16_224': _cfg(url=None,  # 可以是URL，也可以是None,
    pretrained_finetune='/home/lyj/code_lpq/pretrain_finetune/BeiTB/beit_base_patch16_224_pt22k_ft22k.pth',),

}
# '/home/lyj/MFM/models/ViTB16_FAG.pth','/home/lyj/code_lpq/pretrain_finetune/vit_base_p16_224-4e355ebd.pth'
# /home/data1/lyj/CMA/Output_RODHQ/FMAG/ViT/ViT-0.pth
# cfg = {
#     'url': None,  # 可以是URL，也可以是None
#     'pretrained_finetune': '/home/lyj/code_lpq/pretrain_finetune/vit_base_p16_224-4e355ebd.pth',
#     'num_classes': 2, 'input_size': (3, 224, 224), 'pool_size': None,
#     'crop_pct': .9, 'interpolation': 'bicubic',
#     'mean': (0.485, 0.456, 0.406), 'std': (0.229, 0.224, 0.225),
...
class VisionTransformerBat(nn.Module):
    ...
    def __init__(self,model_name='', img_size=224, patch_size=16, patch_size_prompt=16,in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 new_patch_size=None, adapter_type=None,pretrain=True,add_cls_token=True,adapter_dim=16,scale_factor=16):
        """
        Args:
        ...
        """
        # super().__init__()
        super(VisionTransformerBat, self).__init__()
        ...
        #     img_size=img_size, patch_size=patch_size_prompt, in_chans=in_chans, embed_dim=embed_dim)
        self.patch_embed_prompt_FMAG = PatchEmbed_prompt(
            img_size=img_size, patch_size=patch_size_prompt, in_chans=in_chans, embed_dim=embed_dim)
        # 统一生成共享选择
        # self.shared_choice = random.choices(choices, weights=weights, k=1)[0]
        ...
        for i in range(depth):
            ...
            if i < 20:
                ...
                #         keep_ratio_search=ce_keep_ratio_i,adapter_dim=adapter_dim,scale_factor=scale_factor)
                # )
                blocks.append(
                    CEABlock(
                        dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
        ...
        self.adapter_up = nn.Linear(768 // scale_factor, 768)
        # self.adapter_up2 = nn.Linear(768 // scale_factor, 768)
    ...
    # def forward_features(self,x_otsu, z_otsu,z, x, mask_z=None, mask_x=None,
    ...
    def forward_features(self, x_otsu, z_otsu,x_FMAG, z, x, mask_z=None, mask_x=None,
        ...
        z_otsu = zi
        x_otsu = x
        # z_otsu = self.patch_embed_prompt_CMA(z_otsu)
        # x_otsu = self.patch_embed_prompt_rgb(x_otsu)
        # x_origin = self.patch_embed_prompt_rgb(x_rgb)
        x_origin = None
        # zi_origin = self.patch_embed_prompt_CMA(z_rgb)
        # x_origin = self.patch_embed_prompt_rgb(x_rgb,chosen_filter)
        # zi_origin = self.patch_embed_prompt_CMA(z_rgb,chosen_filter)
        zi_origin = zi
        ###################################################################===========
        ...
        #     cls_token_mask = torch.zeros(mask_z.shape[0], 1, dtype=torch.bool, device=mask_z.device)
        #     mask_z = torch.cat([cls_token_mask,mask_z], dim=1)  # 在第一个位置添加 CLS token 的 mask，变为 [16, 257]
        if mask_z is not None:
            # Perform interpolation to adjust the spatial size
            ...
            # if mask_z.dim() == 2:
            #     mask_z = mask_z[:, None, :]  # Expand to [B, 1, 1, 257] if needed
        ...
        # x_origin = x
        # zi_origin = zi
        # x = combine_tokens(x, x, mode='direct')  ##[Batch size, 320, 768]
        # x = combine_tokens(x, zi, mode='direct')
        # zi = combine_tokens(x_origin, zi_origin, mode='direct')
        # z = combine_tokens(z, z, mode=self.cat_mode)

        # zi = combine_tokens(z_otsu, x_otsu, mode='direct')
        # zi = combine_tokens(z, z, mode=self.cat_mode)
        if self.add_cls_token:
        ...
        removed_flag = False
        use_depth = False
        adapter_up = self.adapter_up
        adapter_up2 = self.adapter_up2
        # 在调用 blk 时只传入必要的参数
        for i, blk in enumerate(self.blocks):
            x, attn, zi, attn_i = blk(x_otsu,z_otsu,x_FMAG,x,zi,x_origin,zi_origin,
                                          adapter_up,mask_x,mask_z) ##FMAG
    ...
    def forward(self, x, x_FMAG=None,ce_template_mask=None, ce_keep_rate=None,
        ...
        # print("x",x.shape)
        x, aux_dict = self.forward_features(x_stsu, z_otsu, x_FMAG,z, x,mask_z=mask_reverse,mask_x=mask, ce_template_mask=ce_template_mask,
    ...
    def IIC_sort(self, x):
        ...
        # b_channel, g_channel, r_channel = x.unbind(dim=1)
        # r_channel, g_channel, b_channel = x_inverse.unbind(dim=1)
        # 计算色度
